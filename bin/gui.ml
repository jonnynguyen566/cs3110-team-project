...
let () =
  ...
  let beginning = W.image ~w:1500 ~h:1050 ~noscale:true "images/click_to_begin.png" in
  ...
  let instructions = W.image ~w:1500 ~h:1050 ~noscale:true "images/instructions.png" in
  ...
  W.connect_main beginning beginning (fun ev x y ->
    if !current_state = Intro1 then transition_to_intro2 ev x y
  ) Trigger.buttons_up 
  |> W.add_connection beginning;

  W.connect_main instructions instructions (fun ev x y ->
